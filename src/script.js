...
/**
 * Find the four neighbors around (above, below, right, and left) an array element.
 * 
 * Only consider [a-z] as a valid neighbor. 
 * 
 * If the neighbor is [A-Z], it will locate the next available [a-z] and mark it as 
 * the neighbor instead. 
 * 
 * If the neighbor is [-], it will be marked `false` as a neighbor, and will find 
 * the next available [a-z] after it to be marked as 'jump' neighbor.
 * 
 */
...
/**
 * Add behaviours to active inputs in crossword, including:
 *  - Automatically move input focus when typed in or deleted
 *  - Change typing flow of an 'intersectional' input from cross 
 *    to down and vice versa through click
 *  - Move through inputs using arrows
 *  - Doing actions when all inputs are filled, and check whether
 *    the anwers are correct or not
 * 
 */
